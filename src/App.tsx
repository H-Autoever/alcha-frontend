...
function App() {
  ...
  const vehicle: {
    ...
    state: vehicleData.ignition && vehicleData.gear.gear === 'D' ? 'driving' : 'parked',
  ...
  return (
    <div className="max-w-xl mx-auto p-10">
      <header className="mb-3 flex items-center justify-between px-2">
        <h1 className="text-xl font-bold text-h-blue">{vehicle.vehicle_type}</h1>
        <div className="relative">
          ...
          {recentOpen && (
            <div className="absolute right-0 top-10 z-20 w-72 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
              <div className="bg-h-blue px-3 py-2.5 font-semibold text-white">최근 알림</div>
              <div className="p-3">
                {recent.map((r) => (
                ...
                <button
                  onClick={() => { setRecentOpen(false); navigate('/notifications') }}
      ...
      <section className="relative rounded-2xl border p-4 border-h-sand bg-h-light-blue">
        <div className="flex items-center justify-between gap-50">
          <div className="absolute left-3 top-3 flex items-center gap-2" aria-label={vehicle.state ? 'ignition on' : 'ignition off'}>
            <div className={`h-3 w-3 rounded-full ${vehicle.state === 'driving' ? 'bg-h-green' : 'bg-h-red'}`} />
            <span className="text-sm text-slate-500 inline-block">{vehicle.state === 'driving' ? '주행 중' : '주차 중'}</span>
          </div>
        </div>

        <div className="absolute right-3 top-2 font-semibold">{vehicle.speed} km/h</div>
      ...
      <section className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
        {[
          ...
          <div key={k} className="rounded-xl border p-3 border-h-sand bg-h-white">
